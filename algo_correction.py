    ...
    if not dico_C1_C2_S1 :
        ...
        cliques_par_nom_sommets_new = \
            mise_a_jour_aretes_cliques(
                                nom_sommet_z = nom_sommet_1,
                                set(cliques_couvertures).copy(), 
                                set(aretes_LG_k_alpha_cor).copy(), 
                                aretes_ps,
                                noms_sommets_1.copy(),
                                cliques_par_nom_sommets.copy())
        ...
        for k_c1_c2_s1, val_cpt_c1_c2_s1 in dico_C1_C2_S1.items():
            ...
            if p1 is not None and p2 is not None :
                ...
                cliques_par_nom_sommets_new = \
                    mise_a_jour_aretes_cliques(
                                nom_sommet_z = nom_sommet_1,
                                set(cliques_couvertures_new).copy(), 
                                set(aretes_LG_k_alpha_new).copy(), 
                                aretes_ps,
                                noms_sommets_1.copy(),
                                cliques_par_nom_sommets.copy())
...
#                                               cliques_sommet_1,
        ...
        min_c1, max_c2, dico_sol_C2_C1 = critere_C2_C1_global(
                                            dico_compression,
                                            mode_correction,
                                            critere_correction)                 # C2 : nombre maximum de voisins corriges par un sommet, C1 : nombre minimum d'aretes a corriger au voisinage d'un sommet
        ...
        if not dico_sol_C2_C1 :
            ...
            cliques_par_nom_sommets = fct_aux.grouped_cliques_by_node(
                                            cliques_couv_new);
            cliques_couvertures_cor = list(it.chain.from_iterable(
                                        cliques_par_nom_sommets.values()))
...
###############################################################################
def correction_algo(cliques_couverture,
                    aretes_LG_k_alpha,
                    sommets_LG,
                    mode_correction,
                    critere_correction,
    ...
    cliques_couverture : cliques trouvees + aretes non supprimees
    ...
    """
    #TODO a coder
    cliques_couverture_cor = cliques_couverture.copy()
